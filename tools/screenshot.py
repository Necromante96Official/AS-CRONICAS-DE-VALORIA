...
def main() -> int:
    ...
    try:
        ...
        cdp.eval("""(async () => {
          const g = window.valoria;
          g.player.x = 33*32+4; g.player.y = 44*32; g.player.dir = 'down';
          g.camera.snap(g.player.cx, g.player.cy);
          await new Promise(r => setTimeout(r, 1500));
          return 'ok';
        })()""")
        cdp.shot(os.path.join(SHOTS, "beach.png"))

        print("snow...")
        cdp.eval("""(async () => {
          const g = window.valoria;
          g.player.x = 30*32+4; g.player.y = 3*32; g.player.dir = 'up';
          g.camera.snap(g.player.cx, g.player.cy);
          await new Promise(r => setTimeout(r, 1500));
          return 'ok';
        })()""")
        cdp.shot(os.path.join(SHOTS, "snow.png"))
        print("OK")
        return 0
    finally:
        if browser:
